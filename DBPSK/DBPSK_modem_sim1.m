...
% bchcode  % BCH编码函数
%差分编码
msg_source_diffcode = zeros(1,960);
if msg_source(1) == 1
    msg_source_diffcode(1) = 1;
end
for i = 2:length(msg_source)
    if msg_source(i) == msg_source(i-1)
        msg_source_diffcode(i) = 0;
    else 
        msg_source_diffcode(i) = 1;
    end
end
%%%%组帧
frame_msg = [frame_head msg_source_diffcode];
...
%%%%% 调制器
%%% 双极性变换 
bipolar_msg_source = 2*frame_msg - 1; %将0,1序列变换为-1,1序列
...
%%%提取帧信息
frame_msg_demod = msg_source_MF_option_sign(41:end);
%%%差分译码
msg_source_diffcode_demod = ones(1,length(frame_msg_demod));
if frame_msg_demod(1) == -1
    msg_source_diffcode_demod(1) = -1;
else
    msg_source_diffcode_demod(1) = 1;
end
for i = 2:length(frame_msg_demod)
    if frame_msg_demod(i) == -1
        msg_source_diffcode_demod(i) = msg_source_diffcode_demod(i-1);
    else 
        msg_source_diffcode_demod(i) = -msg_source_diffcode_demod(i-1);
    end
end
...
%%%%%         信宿     %%%%%%  
%%% 误码性能比对
[err_number, bit_err_ratio] = biterr(msg_source(1:length(msg_source_diffcode_demod)), (msg_source_diffcode_demod + 1)/2);
err_number
bit_err_ratio
